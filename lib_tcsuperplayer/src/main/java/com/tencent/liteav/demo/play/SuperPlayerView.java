...
public class SuperPlayerView extends RelativeLayout implements ITXVodPlayListener, ITXLivePlayListener {
    ...
    public VideoStartListener videoStartListener;
    ...
    public void setGotoCharge(gotoCharge gotoCharge) {
    ...
    private void initView(Context context) {
        ...
        mVodControllerLarge.setGotoCharge(new TCVodControllerLarge.gotoCharge() {
            @Override
    ...
    public void playWithModel(final SuperPlayerModel modelV3) {
        ...
        if (modelV3.videoId != null) { // 根据FileId播放
            ...
            if (isRTMPPlay(videoURL)) {       // 直播播放器：普通RTMP流播放
                ...
                playLiveURL(videoURL, TXLivePlayer.PLAY_TYPE_LIVE_RTMP_ACC);
...
//    }
    ...
    private void pause() {
        if (mCurrentPlayType == SuperPlayerConst.PLAYSTATE_PLAY) {
            if (mVodPlayer != null) {
                // 解决Widevine有声音画面不动问题
        ...
        public void onRequestPlayMode(int requestPlayMode) {
            ...
            if (requestPlayMode == SuperPlayerConst.PLAYMODE_FULLSCREEN) {
                ...
                if (mPlayerViewCallback != null) {
                    mVodControllerLarge.setDanmuImage(true);
                    onDanmuku(true);
                    mPlayerViewCallback.onStartFullScreenPlay();
                }
                mVodControllerLarge.showBar(true);
            }
            // 请求窗口模式
                ...
                // 当前是全屏模式
                    ...
                    if (mPlayerViewCallback != null) {
                        mVodControllerLarge.setDanmuImage(false);
                        onDanmuku(false);
...
//                    playWithMode(mCurrentSuperPlayerModel);
        ...
        @Override
        public void senDanmu(String danmu) {
            mDanmuView.addDanmaku(danmu,false);
        }
        ...
        public void onDoubleTap() {
            if (mPlayMode == SuperPlayerConst.PLAYMODE_FULLSCREEN) {
                ...
                onRequestPlayMode(SuperPlayerConst.PLAYMODE_FULLSCREEN);
            }
        }
        ...
        public void showGift(GiftData data) {
            if(mPlayerViewCallback!=null){
            mPlayerViewCallback.showGift(data);
            }
        }
        ...
        public void hideGiftList() {
            if(mPlayerViewCallback!=null){
                mPlayerViewCallback.hideCahtGIftList();
            }
...
//            mVodControllerLarge.updateVideoProgress(progress / 1000, duration / 1000);
    ...
    public void onPlayEvent(int event, Bundle param) {
        Log.e("mLivePlayer_onPlayEvent",param.toString());
        Log.e("mLivePlayer_onPlayEvent",event+"");
        Log.e("mLivePlayer_onPlayEvent","");
        if (event != TXLiveConstants.PLAY_EVT_PLAY_PROGRESS) {
        ...
        if (event == TXLiveConstants.PLAY_EVT_VOD_PLAY_PREPARED) { //视频播放开始
            ...
            if (mCurrentPlayType == SuperPlayerConst.PLAYTYPE_LIVE_SHIFT) {  // 直播时移失败，返回直播
                ...
                mVodControllerSmall.updateReplay(true);
                mVodControllerLarge.updateReplay(true);
                //视频因为意外结束播放,释放资源等待下一次连麦

                if(joinListener!=null){
                    joinListener.joinClose();
                }
                if(videoStartListener!=null){
                    videoStartListener.videoFail();
                }
...
//        }
    ...
    public void hideGiftList() {
        mVodControllerLarge.hideGiftList();
    }

    public void isShowGiftList() {
        mVodControllerLarge.hideGiftList();
    }

    public void setGiftList(ArrayList<GiftData> giftList) {
        mVodControllerLarge.setGiftList(giftList);
    }

    public void addDanmu(String message) {
    ...
    public interface OnSuperPlayerViewCallback {
        ...
        void showGift(GiftData data);

        void hideCahtGIftList();
    ...
    public void cleanClick(){
        mVodControllerLarge.cleanClick();

        mVodControllerSmall.cleanClick();
        mVodControllerFloat.cleanClick();
    }
}
