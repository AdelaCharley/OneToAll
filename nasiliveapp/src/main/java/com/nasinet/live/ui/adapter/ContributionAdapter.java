...
public class ContributionAdapter extends RecyclerView.Adapter<ContributionAdapter.ViewHolder> {
    ...
    public void onBindViewHolder(ViewHolder holder, int position) {
        ...
        if (contributeRank.getUser().getVip_level() == null) {
            holder.iv_vip.setVisibility(View.GONE);
        } else {
            if (contributeRank.getUser().getVip_level().equals("0")) {
                holder.iv_vip.setVisibility(View.GONE);
            } else if (contributeRank.getUser().getVip_date() != null) {
                if (MyUserInstance.getInstance().OverTime(contributeRank.getUser().getVip_date())) {

                    Glide.with(context).load(MyUserInstance.getInstance().getVipLevel(contributeRank.getUser().getVip_level())).into(holder.iv_vip);
                } else {
                    holder.iv_vip.setVisibility(View.GONE);
                }
        ...
        holder.tv_heart.setText(contributeRank.getIntimacy());
        Glide.with(context).load(MyUserInstance.getInstance().getLevel(contributeRank.getUser().getUser_level())).into(holder.iv_user_level);
