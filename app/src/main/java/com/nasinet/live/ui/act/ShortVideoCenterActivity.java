...
public class ShortVideoCenterActivity extends BaseMvpActivity<HomePresenter> implements HomeContract.View {
    ...
    @Override
    public void checkAttent(BaseResponse response) {

        LinkedTreeMap data = (LinkedTreeMap) response.getData();
        Double result = (Double) data.get("attented");
        int temp = new Double(result).intValue();
        if (temp == 0) {
    ...
    private void init(UserInfo bean) {
        ...
        if (!TextUtils.isEmpty(author.getIsattent()) && author.getIsattent().equals("1")) {
            type = "0";
            iv_attention.setImageResource(R.mipmap.personal_center_yiguanzhu);
        ...
        iv_user_level.setImageResource(MyUserInstance.getInstance().getLevel(author.getUser_level()));
        age_tv.setText(author.getProfile().getAge());
        if (author.getProfile().getGender().equals("1")) {
        ...
        rl_chat.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if (MyUserInstance.getInstance().visitorIsLogin()) {
                    MyUserInstance.getInstance().startChatActivity(ShortVideoCenterActivity.this, author.getId(), author.getNick_name(), ChatActivity.class);
                }
            }
        });
    ...
    protected void initView() {
        ...
        author = (Author) intent.getSerializableExtra("authorInfo");
    }
