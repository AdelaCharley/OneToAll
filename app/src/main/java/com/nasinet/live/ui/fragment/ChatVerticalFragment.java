...
public class ChatVerticalFragment extends BaseMvpFragment<SuperPlayerPresenter> implements SuperPlayerContrat.View {
    ...
    private void checkAttent() {
        if (MyUserInstance.getInstance().hasToken()) {
            ...
            HttpUtils.getInstance().checkAttent(hotLive.getAnchorid() + "", new StringCallback() {
                @Override
                public void onSuccess(Response<String> response) {
                    //StringUtil.log("json:" + response.body());
    ...
    @OnClick({R.id.v_left, R.id.v_right, R.id.garudian_tv, R.id.chat_join_iv, R.id.ll_no_talk, R.id.chat_gouwuche_iv, R.id.rl_shop_item, R.id.iv_shop_close, R.id.chat_et, R.id.iv_share, R.id.chat_mess_iv, R.id.head_iv, R.id.close_iv, R.id.chat_gitf_iv, R.id.contribution_tv, R.id.follow_iv/*, R.id.chat_gitf_ll*//*, R.id.atv_ply_v_root_ll, R.id.atv_ply_v_send_chat_tv*/})
            ...
            case R.id.chat_mess_iv:
                if (MyUserInstance.getInstance().visitorIsLogin(getActivity())) {
                    new XPopup.Builder(getContext())
                            ...
                            .asCustom(new ChatPopup(getContext())/*.enableDrag(false)*/)
                            .show();
                }
                break;
            case R.id.iv_share:
                ...
                if (MyUserInstance.getInstance().visitorIsLogin(getActivity())) {
                    if (shopItems == null) {
                        ...
                        new XPopup.Builder(getContext())
                                .autoDismiss(false)
                                .moveUpToKeyboard(false) //如果不加这个，评论弹窗会移动到软键盘上面
                                .asCustom(commentShopList/*.enableDrag(false)*/)
                                .show();
                    }
                }
                break;
    ...
    @Override
    public void getGoodsList(BaseResponse<ArrayList<ShopItem>> baseResponse) {
