...
public class RoomNoTalkAdapter extends RecyclerView.Adapter<RoomNoTalkAdapter.ViewHolder> {
    ...
    public RoomNoTalkAdapter(ArrayList<UserRegist> list, Context context) {
    ...
    public void onBindViewHolder(ViewHolder holder, int position) {
        ...
        if (userRegist.getVip_level() == null) {
            ...
            if (userRegist.getVip_level().equals("0")) {
                holder.iv_vip.setVisibility(View.GONE);
            } else if (userRegist.getVip_date() != null) {
                if (MyUserInstance.getInstance().OverTime(userRegist.getVip_date())) {
                    Glide.with(context).load(MyUserInstance.getInstance().getVipLevel(userRegist.getVip_level())).into(holder.iv_vip);
                } else {
                    holder.iv_vip.setVisibility(View.GONE);
                }
            } else {
        ...
        Glide.with(context).load(MyUserInstance.getInstance().getLevel(userRegist.getUser_level())).into(holder.iv_user_level);
        holder.tv_name.setText(userRegist.getNick_name());


        Glide.with(context).load(R.mipmap.cancel_can_talk).into(holder.iv_cancel);
