...
public class AttendUserAdatper extends RecyclerView.Adapter<AttendUserAdatper.ViewHolder> {
    ...
    public AttendUserAdatper(List <AttentUser> attentAnchors, Context context){
    ...
    public void onBindViewHolder(ViewHolder holder, int position){
        ...
        AttentUser attentAnchor = attentAnchors.get(position);
        ...
        if(attentAnchor.getIsattent()==null){
            attentAnchor.setIsattent("0");
        }
        if(attentAnchor.getIsattent().equals("1")){
        ...
        holder.iv_follow.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                int attent=0;
                if(attentAnchor.getIsattent().equals("0")){
                    attent=1;
        ...
        Glide.with(context).load(MyUserInstance.getInstance().getLevel(attentAnchor.getUser_level())).into(holder.iv_user_level);
        if(attentAnchor.getAnchor_level()!=null){
            Glide.with(context).load(MyUserInstance.getInstance().getAnchorLevel(attentAnchor.getAnchor_level())).into(holder.iv_anchor_level);
