...
import com.nasinet.live.widget.MyImageSpan;
...
public class ChatListVerticalViewerAdapter extends RecyclerView.Adapter<ChatListVerticalViewerAdapter.MyViewHolder> {
    ...
    public ChatListVerticalViewerAdapter(List<Message> infos, Context context) {
        this.infos = infos;
    ...
    public MyViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        ...
        MyViewHolder holder = new MyViewHolder(LayoutInflater.from(context).inflate(R.layout.chat_list_item_2, parent,
                false));
    ...
    public void onBindViewHolder(MyViewHolder holder, int position) {
        ...
        if (position == 0) {
            ...
            if (infos.get(position).getAction().equals("RoomMessage") || infos.get(position).getAction().equals("RoomAttentAnchor")) {
                ...
                //用户等级
                builder.insert(0, " ");
                Drawable d_lv = context.getResources().getDrawable(MyUserInstance.getInstance().getLevel(infos.get(position).getData().getChat().getSender().getUser_level() + ""));
