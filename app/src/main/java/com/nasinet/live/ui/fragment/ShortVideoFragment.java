...
import com.nasinet.live.ui.act.ShortVideoCenterActivity;
...
import com.nasinet.live.ui.adapter.ShortVideoAdapter;
import com.nasinet.live.widget.CommentPopup;
import com.nasinet.live.widget.pagerlayoutmanager.OnViewPagerListener;
import com.nasinet.live.widget.pagerlayoutmanager.ViewPagerLayoutManager;
...
public class ShortVideoFragment extends BaseMvpFragment<HomePresenter> implements HomeContract.View, OnViewPagerListener, OnLoadMoreListener, ShortVideoAdapter.CommentListener {
    ...
    public void onPageSelected(int position, boolean isBottom) {
        ...
        if(viewHolder!=null) {
            ...
            if (null == videoAdapter.dataList.get(position).getAuthor().getIsattent()) {
                Glide.with(getContext()).load(R.mipmap.short_guanzhu).into(viewHolder.im_guanzhu);
            } else {
                if (videoAdapter.dataList.get(position).getAuthor().getIsattent().equals("0")) {
        ...
        for (String key : hs_attend.keySet()) {
            if (key.equals(author_id)) {
                for (int i = 0; i < hs_attend.get(key).size(); i++) {
                    videoAdapter.dataList.get(Integer.parseInt(hs_attend.get(key).get(i).toString())).getAuthor().setIsattent(type);
                }
                break;
            }
        }
