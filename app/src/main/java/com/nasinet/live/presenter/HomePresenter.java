...
import com.nasinet.live.model.HomeModel;
...
//View是否绑定 如果没有绑定，就不执行网络请求
    ...
    public void getAttentAnchors(int page) {
        ...
        model.getAttentAnchors(new FormBody.Builder().add("page", page + "")
                ...
                .add("token", MyUserInstance.getInstance().getUserinfo().getToken())
                .add("size", "20").build())
                .compose(RxScheduler.<BaseResponse<ArrayList<AttentUser>>>Flo_io_main())
                .as(mView.<BaseResponse<ArrayList<AttentUser>>>bindAutoDispose())
                .subscribe(new Consumer<BaseResponse<ArrayList<AttentUser>>>() {
                    @Override
                    public void accept(BaseResponse<ArrayList<AttentUser>> bean) throws Exception {
                        if (bean.isSuccess()) {
