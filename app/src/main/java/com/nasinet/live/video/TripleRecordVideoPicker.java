package com.nasinet.live.video;
...
public class TripleRecordVideoPicker extends AbsPickerUI implements ActivityCompat.OnRequestPermissionsResultCallback {
    ...
    public void initDefault() {
        // 设置标题
        getTitleBar().setTitle(getResources().getString(com.tencent.qcloud.ugckit.R.string.video_choose)+"(至少15秒)", ITitleBarLayout.POSITION.MIDDLE);
        ...
        if (ContextCompat.checkSelfPermission(mActivity, Manifest.permission.READ_EXTERNAL_STORAGE) == PackageManager.PERMISSION_GRANTED) {
            mHandlder.post(new Runnable() {
                ...
                public void run() {
                    ...
                    ArrayList<TCVideoFileInfo> list_change=new ArrayList<>();
                    for(int i=0;i<list.size();i++){
                        if(list.get(i).getDuration()>15000){
                            list_change.add(list.get(i));
                        }
                    }
                    getPickerListLayout().updateItems(list_change);
                }
            });
        } else {
