...
//        uploadUserInfo(callback);
    ...
    public void getVodSig2( String uid,String token, final StringCallback callback) {
        try {



            String reqUrl = "https://live.nasinet.com/api/config/getSignForVod";
            OkGo.<String>post(reqUrl)
                    .tag(this)
                    .params("uid", uid)
                    .params("token",  token)
    ...
    private void request2(String cmd, String body, okhttp3.Callback callback) {

        String reqUrl = "http://live.nasinet.com/" + cmd;
        Request request = new Request.Builder()
                .url(reqUrl)
                .post(RequestBody.create(MediaType.parse("application/json; charset=utf-8"), body))
                .build();
